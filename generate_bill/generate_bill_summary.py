...
try:
    opts, args = getopt.getopt(
        sys.argv[1:],
        "hs:e:f:k:u:t:",
        ["start_date=", "end_date=", "output_format=", "api_key=", "username=", "type="],
        )
except getopt.GetoptError:
    print("%s -s <start_date> -e <end_date> -f <pdf_or_xls> -k <api_key> -u <username> -t <invoice-type>" % sys.argv[0])
    print("example:")
    print("         %s -s 03/01/2022 -e 03/30/2022 -f pdf -k my-api-key -u test" % sys.argv[0])
    print("")
    sys.exit(2)
for opt, arg in opts:
    if opt == '-h':
        print("%s -s <start_date> -e <end_date> -f <pdf_or_xls> -k <api_key> -u <username> -t <invoice-type>" % sys.argv[0])
        print("example:")
        print("         %s -s 03/01/2022 -e 03/30/2022 -f pdf -k my-api-key -u test -t ALL" % sys.argv[0])
        print("")
        sys.exit()
...
class SL_Service():
    ...
    def get_invoices(self, startDate, endDate):
        objectMask = "mask[id, createDate, typeCode, amount, invoiceTotalAmount, invoiceTotalOneTimeAmount, invoiceTotalOneTimeTaxAmount, invoiceTotalPreTaxAmount, invoiceTotalRecurringAmount, invoiceTotalRecurringTaxAmount, payment, startingBalance, endingBalance]"
        objectFilter = {
            'invoices': {
                'createDate': {
